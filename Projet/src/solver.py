...
# def compute_conservation_of_energy(T, input_dict, bc_top_tinf=None, order=2):
#     ny = input_dict['ny']
#     nx = input_dict['nx']
#     kappa = input_dict['k']
#     rho = input_dict['rho']
...
#     # Intérieurs seulement sur les côtés verticaux
#     y_side = y[1:-1]
#     u_side = u_y[1:-1]

#     if order == 1:
#         dTdx_left  = (T[1:-1, 1] - T[1:-1, 0]) / dx
#         dTdx_right = (T[1:-1, -1] - T[1:-1, -2]) / dx
#     elif order == 2:
#         dTdx_left  = (-3.0*T[1:-1, 0] + 4.0*T[1:-1, 1] - T[1:-1, 2]) / (2.0*dx)
#         dTdx_right = ( 3.0*T[1:-1, -1] - 4.0*T[1:-1, -2] + T[1:-1, -3]) / (2.0*dx)
#     else:
#         raise ValueError("order must be 1 or 2")

#     flux_left = kappa * dTdx_left - rho * cp * u_side * T[1:-1, 0]
#     flux_right = -kappa * dTdx_right + rho * cp * u_side * T[1:-1, -1]

#     left_contrib = np.trapz(flux_left, y_side)
#     right_contrib = np.trapz(flux_right, y_side)

#     # Bas : Neumann homogène
#     bottom_contrib = 0.0

#     # Haut : exclure les coins, car ils sont Dirichlet dans le solveur
#     x_top = x[1:-1]
#     T_top = T[-1, 1:-1]

#     if bc_top_tinf is None:
#         Tinf_top = tinf * np.ones_like(x_top)
#     else:
#         Tinf_top = bc_top_tinf(x_top)

#     flux_top = h * (T_top - Tinf_top)
#     top_contrib = np.trapz(flux_top, x_top)

#     source_contrib = f * b * c

#     total = left_contrib + right_contrib + bottom_contrib + top_contrib - source_contrib

#     print(f"left   = {left_contrib:.12e}")
#     print(f"right  = {right_contrib:.12e}")
#     print(f"bottom = {bottom_contrib:.12e}")
#     print(f"top    = {top_contrib:.12e}")
#     print(f"source = {source_contrib:.12e}")
